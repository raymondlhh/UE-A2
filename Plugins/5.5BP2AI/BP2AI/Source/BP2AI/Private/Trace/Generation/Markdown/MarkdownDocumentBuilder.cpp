...
void FMarkdownDocumentBuilder::ProcessStructuredContent(const FTracingResults& TracingData, TArray<FString>& OutLines)
{
    ...
    UE_LOG(LogBP2AI, Warning, TEXT("MarkdownDocumentBuilder (!!!MARKDOWN!!!): Available visible definition categories: %d"), DefinitionsByCategory.Num());
    for (const auto& CategoryPair : DefinitionsByCategory)
    {
        UE_LOG(LogBP2AI, Warning, TEXT("  '%s': %d definitions"), 
            *CategoryPair.Key, CategoryPair.Value.Num());
    }

    UE_LOG(LogBP2AI, Warning, TEXT("MarkdownDocumentBuilder (!!!MARKDOWN!!!): Processing sections in ExecutePrintingPhase order:"));
    
    for (const FSectionMetadata& Section : TracingData.SectionHeaders)
    {
        UE_LOG(LogBP2AI, Warning, TEXT("  Processing: '%s' (Level %d, Major: %s)"), 
        ...
        {
            UE_LOG(LogBP2AI, Warning, TEXT("    ?? SKIPPED: Category '%s' is not visible"), *Section.SectionName);
            continue;
        }
        ...
        if (Section.Level == 3)
        {
            if (DefinitionsByCategory.Contains(Section.SectionName))
            {
                ...
                UE_LOG(LogBP2AI, Warning, TEXT("    ? MATCHED: Adding %d definitions for '%s'"), 
            ...
            {
                UE_LOG(LogBP2AI, Warning, TEXT("    ?? NO DEFINITIONS: Category '%s' has no definitions to add"), 
                    *Section.SectionName);
            }
        }
    }
    ...
    {
        UE_LOG(LogBP2AI, Warning, TEXT("MarkdownDocumentBuilder (!!!MARKDOWN!!!): ? SUCCESS: All definitions properly matched to section headers"));
    }
}
